	...
	function custom_post_type_page_template_admin_init() {
		...
		if ( !empty($options['post_types']) && is_array($options['post_types']) ) :
			foreach( $options['post_types'] as $post_type ) :
				add_meta_box( 'pagetemplatediv', __('Modelo de Página', 'custom-post-type-page-template'), array(&$this, 'custom_post_type_page_template_meta_box'), $post_type, 'side', 'core');
			endforeach;
		endif;
	}

	function custom_post_type_page_template_admin_menu() {
		add_options_page( __('Custom Post Type Page Template', 'custom-post-type-page-template'), __('Templates', 'custom-post-type-page-template'), 'manage_options', basename(__FILE__), array(&$this, 'custom_post_type_page_template_options_page') );
	}

	function custom_post_type_page_template_meta_box($post) {
		$template = get_post_meta($post->ID, '_wp_page_template', true);
?>
<label class="screen-reader-text" for="page_template"><?php _e('Modelo de Página', 'custom-post-type-page-template') ?></label><select name="page_template" id="page_template">

<option value='default'><?php _e('Página Padrão', 'custom-post-type-page-template'); ?></option>
...
<h2><?php _e('Modelo de Página', 'custom-post-type-page-template'); ?></h2>
...
<p class="submit"><input type="submit" value="<?php _e('Salvar', 'custom-post-type-page-template'); ?>" class="button-primary" id="submit" name="submit"></p>
</form>
<?php
	}
}
